...
import { getDeliveries } from 'apiSdk/deliveries';
...
function DashboardPage() {
  ...
  const deliveryStat = useSWR<PaginatedInterface<DeliveryInterface>>(
    () => `/deliveries`,
    () => getDeliveries({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Deliveries',
      result: deliveryStat?.data?.totalCount,
      isLoading: deliveryStat.isLoading,
