...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  ...
  'ticket_opened.create': {
    roles: ['business-owner', 'customer-service-representative'],
    key: 'ticket_id',
    tenantPath: [],
    userPath: [],
  },
  'inventory_low.update': { roles: ['business-owner'], key: 'product_id', tenantPath: [], userPath: [] },
};

const ownerRoles: string[] = ['business-owner'];
const customerRoles: string[] = [];
const tenantRoles: string[] = ['business-owner', 'customer-service-representative', 'end-customer'];
